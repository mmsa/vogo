...
def upsert_membership(db: Session, name: str, provider_slug: str) -> Membership:
    """Create or update a membership."""
    membership = db.query(Membership).filter(
        Membership.provider_slug == provider_slug
    ).first()
    
    if membership:
        membership.name = name
    else:
        membership = Membership(name=name, provider_slug=provider_slug)
        db.add(membership)
        db.flush()
